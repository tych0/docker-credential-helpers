...
func TestPassHelper(t *testing.T) {
	...
	for server, username := range credsList {
		if !strings.Contains(server, "2376") &&
			!strings.Contains(creds.ServerURL, "9999") {
	...
	err = helper.Delete("https://foobar.docker.io:9999/v2")
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(credsList) != 1 {
		t.Fatal("didn't delete cred?")
	}

	err = helper.Delete("https://foobar.docker.io:2376/v1")
	if err != nil {
		t.Fatal(err)
	}
